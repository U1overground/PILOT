...
class SegmentationMetric(object):
    ...
    def __init__(self, config):
        ...
        # config is expected to be a simple object with attributes
        # I'll create a dummy inner config if needed
        class DummyConfig:
            pass
        ...
        try:
            ...
            self.config.model = DummyModel()
            self.config.model.align_corners = config.MODEL.ALIGN_CORNERS
        except AttributeError:
            print("Warning: Could not find full config structure. Assuming default attributes.")
            ...
            try:
                ...
                print(f"Error setting up SegmentationMetric config: {e}. Defaulting nclass=19, align_corners=True")
                self.nclass = 19 # Default for cityscapes
                self.config.model = DummyModel()
                self.config.model.align_corners = True
    ...
    def update(self, preds, labels):
        ...
        ----------
        labels : 'Tensor'
            The labels of the data (e.g., [N, H, W]).
        preds : 'Tensor'
            Predicted values (e.g., [N, C, H, W] or [N, C, PH, PW]).
        """

        ## reshaping
        h, w = labels.size(1), labels.size(2)
        
        # Check if preds is a list/tuple, which function.py might pass
        # This implementation of 'update' expects a single tensor
        if isinstance(preds, (list, tuple)):
            # Assuming the first prediction is the one we want to evaluate
            # This is a common case if model returns multiple outputs
            preds = preds[0] 
        ...
        if isinstance(preds, torch.Tensor):
            ...
            # This logic in performance.py is a bit ambiguous,
            # it zips preds and labels, implying multiple labels?
            # My update above (taking preds[0]) avoids this.
            # I'll stick to the single tensor logic for clarity.
            print("Warning: List of preds passed to SegmentationMetric.update, evaluating only first element.")
            evaluate_worker(self, preds[0], labels)
...
class SegmentationMetrics2:
    def __init__(self, config):
        ...
        try:
            ...
            print("Warning: Could not find config.DATASET.NUM_CLASSES. Defaulting to 19.")
            self.num_classes = 19

        self.ignore_index = None # This implementation ignores class 255 by logic
    ...
    def update(self, pred, target):
        ...
        Expects pred to be interpolated already.
        ...
        pred_labels[target == 255] = 255  # where target is 255, predlabel is 255
